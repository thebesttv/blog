...
- We can also construct a complex number using $(x, y)$ or $(r, A)$
  with =make-from-real-imag= and =make-from-mag-ang=.
...
Assuming we have implemented the six functions, we can use it to do
complex arithmetic.
#+begin_src scheme
...
As for the rectangular form and plar form, both representations are
implemented with a =cons= pair.
...
- dispatch on data types: have the function decide what to do
  depending on the data type given.
