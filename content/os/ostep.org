...
#+begin_quote
Making information persist, despite computer crashes, disk failures, or power
outages is a tough and interesting challenge.
#+end_quote

A typical system has a hierarchical structure where high performance devices
(e.g., RAM and GPU) are close to CPU and low performance devices are on lower
tiers.

** A Canonical Device & Protocol

A canonical device has a /hardware interface/ to the system and its /internal
implementation/.  The interface is composed of three registers:
- *status* register: tells the current device status (busy, idle, error, ...)
- *command* register: used to send command to device
- *data* register: pass data between device & system

For the OS to make the device do something, a simple protocol:
1. The OS *polls* the status register of the device until it is ready.
2. OS sends data through data register.  Here the CPU itself does the IO, an
   inefficient method called [[https://en.wikipedia.org/wiki/Programmed_input%E2%80%93output][programmed I/O (PIO)]].
3. OS sends command via command register.
4. The device starts processing.
5. OS waits until the device is done by polling its status register again.

This simple protocol has some inefficiencies.

*** Polling wastes CPU time

Instead, the OS can put the waiting process to sleep and use interrupt to
handle device status change while also waking up the sleeping process.  This
allows for overlap of computation and IO.

However, interrupts has some overhead.  If the device can handle request quite
quickly, polling may be better.  Thus, a *hybrid* would be to poll for a little
while, and if the device is not yet finished, use interrupts.  This
*two-phased* approach may achieve the best of both worlds.

Also, if the system is suddenly flooded with many interrupts, it may
livelock---only processing interrupts and not waking up processes to handle the
request.  An optimization would be *coalescing*---the devices merges multiple
interrupts into a single one, thus lowering the system's burden.

*** PIO waits CPU time

When transfering large chunk of data from RAM to IO port, CPU time is again
wasted doing trivial data transfer.  Adding [[https://en.wikipedia.org/wiki/Direct_memory_access][hardware DMA]] enables CPU to
delegate data transfer to a specific hardware device, thus freeing more CPU
time.

** Methods of Device Interaction

[[https://en.wikipedia.org/wiki/Memory-mapped_I/O][Memory-mapped IO & Port-mapped IO]]
- port-mapped IO uses explicit IO instructions on a specific IO port
- memory-mapped IO maps device registers to memory address space, thus only
  regular memory transfer instructions are needed for IO

** Device Driver

Device driver makes OS device-neutral---the details of device interactions are
hidden from major OS subsystems.

* TODO 37. Hard Disk Drives

A disk drive consists of an array of *sectors* (the address space of the drive)
that store user data.  Traditionally, each sector has a fixed size of 512
bytes, but [[https://en.wikipedia.org/wiki/Advanced_Format][Advanced Format]] allows 4KB sectors.

A disk has one or mote *platters*; each platter has 2 sides, each of which is
called a *surface* (盘面).

IO time consists of three parts:
- *seek time* (寻道时间): time head takes seeking the right track.  $T_{seek}$
- *rotational delay* (旋转等待时间): head waits for the correct sector.
  $T_{loop} = 60 \cdot 1000 / \text{RPM} \, \text{ms}$
- *transfer time* (传输时间): time spent reading/writing a single
  sector. $T_{transfer}$

* TODO 38. Redundant Arrays of Inexpensive Disks (RAIDs)

RAID connects multiple disks in concert and uses redundancy to build a large,
fast and reliable storage system, all transparent to the OS.

We assume a simple *fault model*---the *fail-stop* fault model, where a disk
can be either working or failed.  A RAID is evaluated along three axes:
capacity, reliability, and performance.

** RAID 0: Striping

RAID 0 is technically not a RAID as it uses no reduncancy.  It serves only as
an upper-bound on performance and capacity.  Assume there're $N$ disks each
with $B$ blocks.  A RAID array of *chunk size* $k$ blocks will form a *stripe*
of $N \times k$ blocks.  Each chunk sits in one disk.  The stripe spreads
across the $N$ disks.
