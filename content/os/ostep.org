...
*locks* around a critical section ensures that such section is
executed *as if* it were a single atomic instruction.

A lock is either *available* (*unlocked*, or *free*) and thus no
thread holds the lock, or *acquired* (*locked*, or *held*), and thus
*exactly one thread* holds the lock ans presumable is in a critical
section.  This thread is called the owner of the lock.
...
- performance: what's the time overhead added by using the lock under
  different situations?
  1. no contention: only a single thread is running and grabs &
     releases the lock
  2. uni-core: multiple threads are contending for the lock on a
     single CPU
  3. multi-core: multiple threads on multiple CPUs are contending for
     the lock
...
Hardware support:
- test-and-set: allows you to test the old value of a memory location
  while also setting it to the new value


The simple spin lock requires a preemptive scheduler, especially on a
single CPU, as without preemption, the thread trying to grap the lock
